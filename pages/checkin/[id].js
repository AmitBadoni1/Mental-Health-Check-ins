...
const MOVIE_QUESTIONS = [
  "Tell me about your favourite movie. What was its story and characters?",
  "Describe one scene you remember very clearly.",
  "Have you seen other movies of any of the actors?"
];
...
export default function CheckinPage() {
  ...
  useEffect(() => {
    ...
    if (step === "movie" && questionIndexRef.current < MOVIE_QUESTIONS.length - 1) {
      const next = questionIndexRef.current + 1;
      questionIndexRef.current = next;
      setQuestionIndex(next);
      setCurrentPrompt(MOVIE_QUESTIONS[next]);
    }

    if (step === "stress" && questionIndexRef.current < STRESS_QUESTIONS.length - 1) {
      const next = questionIndexRef.current + 1;
      questionIndexRef.current = next;
      setQuestionIndex(next);
      setCurrentPrompt(STRESS_QUESTIONS[next]);
    }
  };
  ...
  const startRecording = async () => {
    ...
    recorder.onstop = () => {
      try {
        ...
        console.log("Saved recording:", blob);
      } catch (e) {
        console.error(e);
      }
        ...
        <div className="bg-white p-6 rounded-lg shadow text-center">
          ...
          {step === "pre-survey" && (
            <>
              ...
              <button
                onClick={() => {
                  setStep("movie");
                  setCurrentPrompt(MOVIE_QUESTIONS[0]);
                  setQuestionIndex(0);
                }}
          ...
          {step === "stress-intro" && (
            <>
              ...
              <button
                onClick={() => {
                  setStep("stress");
                  setCurrentPrompt(STRESS_QUESTIONS[0]);
                  setQuestionIndex(0);
                  setRecordingSeconds(0);
                  setSilenceSeconds(0);
          ...
          {step === "happy" && (
            <>
              ...
              {isRecording && (
                <button
                  onClick={() => stopRecording("thank-you")}
                  className="bg-red-500 text-white w-full py-3 rounded-lg"
                >
                  Stop Recording
                </button>
